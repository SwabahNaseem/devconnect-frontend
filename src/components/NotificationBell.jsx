...
export default function NotificationBell() {
  ...
  // Accept join request directly from notification
  const acceptRequest = async (e, notif) => {
    e.stopPropagation();
    try {
      // Find the pending request for this project by the actor
      const reqsRes = await API.get(`/api/projects/${notif.projectId}/requests`);
      const req = reqsRes.data.find(r => r.userId === notif.actorId);
      if (!req) { alert('Request not found or already handled'); return; }
  ...
  // Decline join request directly from notification
  ...
  const handleNotifClick = async (notif) => {
    ...
    if (notif.projectId) {
      if (notif.type === 'JOIN_REQUEST') {
        navigate(`/projects/${notif.projectId}?tab=requests`);
      } else {
  ...
  return (
    <div ref={bellRef} style={{ position:'relative' }}>
      ...
      <button onClick={handleOpen}
        style={{ position:'relative',background:isDark?'rgba(255,255,255,0.03)':'rgba(0,0,0,0.03)',border:`1px solid ${navBorder}`,borderRadius:8,width:36,height:36,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:textMuted,transition:'all .15s' }}
        ...
        onMouseLeave={e=>{ e.currentTarget.style.background=isDark?'rgba(255,255,255,0.03)':'rgba(0,0,0,0.03)'; e.currentTarget.style.color=textMuted; }}>
        ...
        <div style={{ position:'absolute',right:0,top:46,width:360,background:dropBg,border:`1px solid ${dropBord}`,borderRadius:12,boxShadow:'0 20px 60px rgba(0,0,0,0.25)',zIndex:300,overflow:'hidden' }}>
          ...
          <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 16px',borderBottom:`1px solid ${divider}` }}>
            <div style={{ display:'flex',alignItems:'center',gap:8 }}>
              ...
              {count > 0 && (
                <span style={{ background:'rgba(37,99,235,0.15)',color:'#3b82f6',borderRadius:99,padding:'1px 8px',fontSize:11,fontWeight:700,fontFamily:"'DM Mono',monospace" }}>
                  {count} new
                </span>
              )}
            </div>
          ...
          <div style={{ maxHeight:420,overflowY:'auto' }}>
            {loading && (
              <div style={{ padding:'32px 0',textAlign:'center',color:textMuted,fontSize:13 }}>Loading…</div>
            )}
            ...
            {!loading && notifs.length === 0 && (
              <div style={{ padding:'44px 16px',textAlign:'center' }}>
                ...
                <p style={{ color:textMuted,fontSize:12,marginTop:4,opacity:.7 }}>No notifications yet</p>
              </div>
            )}

            {!loading && notifs.map(n => (
              <div key={n.id}
                style={{ borderBottom:`1px solid ${divider}`,background:!n.isRead?unreadBg:'transparent',transition:'background .15s' }}>

                {/* Main notification row */}
                <div
                  onClick={() => handleNotifClick(n)}
                  style={{ display:'flex',alignItems:'flex-start',gap:12,padding:'12px 16px 8px',cursor:'pointer',position:'relative' }}
                  ...
                  <div style={{ flex:1,minWidth:0 }}>
                    <p style={{ fontSize:13,color:textPri,lineHeight:1.5,margin:0,fontWeight:n.isRead?400:600 }}>
                      {n.message}
                    </p>
                    <div style={{ display:'flex',alignItems:'center',gap:8,marginTop:3 }}>
                      {n.projectName && (
                        <span style={{ fontSize:11,color:'#3b82f6',fontWeight:600 }}>{n.projectName}</span>
                      )}
                ...
                {/* Accept / Decline buttons — only for JOIN_REQUEST */}
                {n.type === 'JOIN_REQUEST' && (
                  <div style={{ display:'flex',gap:8,padding:'0 16px 12px 64px' }}>
                    <button
                      onClick={e => acceptRequest(e, n)}
